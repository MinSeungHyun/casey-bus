...
export function trackBus(today: Date) {
  ...
  runningTimesTMC.forEach((element) => {
    const [currentStop, nextStop, progress] = getBusProgress(
      routeTMC,
      getElapsedTime(element, today),
    );
    // console.log("---------element: "+getElapsedTime(element, today));
    // console.log("---------elasped Time: "+getElapsedTime(element, today));
    // console.log("---------progress: "+progress);
    const currentLocation = getCurrentLocation(
      getPointsBetweenStop(svgPointsTMC, currentStop, nextStop),
      progress,
    );
  ...
  runningTimesH221.forEach((element) => {
    ...
    const currentLocation = getCurrentLocation(
      getPointsBetweenStop(svgPointsH221, currentStop, nextStop),
      progress,
    );
  ...
  runningTimesHovey.forEach((element) => {
    ...
    const currentLocation = getCurrentLocation(
      getPointsBetweenStop(svgPointsHovey, currentStop, nextStop),
      progress,
    );
...
function getRunnigBus(route: Route, today: Date): [string[], number] {
  const todaySchedule =
    checkHoliday(today.getFullYear(), today.getMonth() + 1, today.getDate()) ||
    [6, 7].includes(today.getDay())
      ? route.schedule_holidays
      : route.schedule_weekdays;
...
function getBusProgress(route: Route, elapsedTime: number): [Stop, Stop, number] {
  ...
  for (let i = 0; i < route.stops.length - 1; i++) {
    if (
      route.stops[i].durationFromStart <= elapsedTime &&
      elapsedTime < route.stops[i + 1].durationFromStart
    ) {
      currentStop = route.stops[i];
      nextStop = route.stops[i + 1];
      progress =
        (elapsedTime - route.stops[i].durationFromStart) /
        (route.stops[i + 1].durationFromStart - route.stops[i].durationFromStart);
      break;
    }
  }
...
function getPointsBetweenStop(pathList: SvgPoint, currentStop: Stop, nextStop: Stop) {
  ...
  for (let i = 0; i < pathList.pointList.length; i++) {
    if (
      pathList.pointList[i][0] == currentStop.x &&
      pathList.pointList[i][1] == currentStop.y &&
      pathList.pointList[i][2] == currentStop.durationFromStart
    ) {
      pushOnList = true;
    }
    if (pushOnList) {
    ...
    if (
      pathList.pointList[i][0] == nextStop.x &&
      pathList.pointList[i][1] == nextStop.y &&
      pathList.pointList[i][2] == nextStop.durationFromStart
    ) {
      pushOnList = false;
      break;
    }
...
function getCurrentLocation(pointList: number[][], progress: number) {
  ...
  const finalX =
    pointList[currentPointIndex][0] +
    (pointList[currentPointIndex + 1][0] - pointList[currentPointIndex][0]) * currentProgress;
  const finalY =
    pointList[currentPointIndex][1] +
    (pointList[currentPointIndex + 1][1] - pointList[currentPointIndex][1]) * currentProgress;
...
export function getNextBusTime(
  schedule: string[],
  today: Date,
