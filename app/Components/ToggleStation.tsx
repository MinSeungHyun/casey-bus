...
export default function ToggleStation({
  ...
  isHoliday: boolean;
  onOpenTable: (station: Stop | null, route: any) => void;
}) {
  const routeList =
    selectedStation && selectedStation.intersaction3
      ? [routeTMC, routeH221, routeHovey]
      : selectedStation && selectedStation.intersaction2
        ? [routeH221, routeHovey]
        : selectedStation && routeTMC.stops.some((s) => s.name === selectedStation.name)
          ? [routeTMC]
          : selectedStation && routeH221.stops.some((s) => s.name === selectedStation.name)
            ? [routeH221]
            : selectedStation && routeHovey.stops.some((s) => s.name === selectedStation.name)
              ? [routeHovey]
              : [];
      ...
      <View style={styles.stationBusList}>
        {selectedStation?.revisit ? (
          <View style={[styles.busRow]}>
            <View style={[styles.busButtonBox, { width: 70 }]}></View>
      ...
      <ScrollView style={styles.upcomingBusBlockBox}>
        {selectedStation?.revisit
          ? routeList.map((route, idx) => {
              const nonRevistStop = route.stops.find(
                (s) => s.name === selectedStation.name && !s.revisit,
              );
              const revistStop = route.stops.find(
                (s) => s.name === selectedStation.name && s.revisit,
              );
              const label_first = getNextBusTime(
                isHoliday ? route.schedule_holidays : route.schedule_weekdays,
                now,
                nonRevistStop?.durationFromStart || 0,
              );
              const label_second = getNextBusTime(
                isHoliday ? route.schedule_holidays : route.schedule_weekdays,
                now,
                revistStop?.durationFromStart || 0,
              );
              return (
                <View key={idx} style={[styles.upcomingBusBlock]}>
          ...
          : routeList.map((route, idx) => {
              const label = getNextBusTime(
                isHoliday ? route.schedule_holidays : route.schedule_weekdays,
                now,
                selectedStation?.durationFromStart || 0,
              );
              return (
                <View key={idx} style={[styles.upcomingBusBlock]}>
                  <View key={idx} style={styles.busRow}>
