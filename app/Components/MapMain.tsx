...
function getDistance(touches: NativeTouchEvent[]) {
  ...
  return Math.sqrt(
    (touch1.locationX - touch2.locationX) ** 2 + (touch1.locationY - touch2.locationY),
  );
}
...
export default function MapMain() {
  ...
  const [isHoliday, setIsHoliday] = useState<boolean>(
    checkHoliday(new Date().getFullYear(), new Date().getMonth() + 1, new Date().getDate()),
  );
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: () => {
        if (Date.now() - tapStartRef.current.t < 300) {
          toggleSheet(null);
          toggleTable(null, selectedRoute);
        }
      },
    }),
  ).current;
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 10 }}>⚠️ Notice</Text>

            <Text style={{ fontSize: 14, textAlign: 'center', marginBottom: 20 }}>
              {/* Bus times may not be accurate. Please check with the station if needed. */}
              Bus movement information in this app is estimated based on the fixed timetable. The
              actual bus location may differ from the displayed position. Please use the arrival
              times and locations for reference only.
            </Text>
      ...
      <View style={{ width: VIEW_W, height: VIEW_H - 90 }}>
        <Animated.View style={[styles.box, { backgroundColor: '#f5f7f6' }]} {...pan.panHandlers}>
          <Svg width={VIEW_W} height={VIEW_H} viewBox={`${vx} ${vy} ${vw} ${vh}`}>
            ...
            <DepartingBusBlock
              route={routeTMC}
              onPress={() => {
                toggleTable(routeTMC.stops[routeTMC.stops.length - 1], routeTMC);
              }}
            />
            <DepartingBusBlock
              route={routeH221}
              onPress={() => {
                toggleTable(routeH221.stops[routeH221.stops.length - 1], routeH221);
              }}
            />
            <DepartingBusBlock
              route={routeHovey}
              onPress={() => {
                toggleTable(routeHovey.stops[routeHovey.stops.length - 1], routeHovey);
              }}
            />
                  ...
                  <Circle
                    ...
                    fill={
                      station.intersaction3
                        ? routeTMC.highlighted || routeH221.highlighted
                          ? '#e5393550'
                          : '#e53935'
              ...
              return (
                <G key={`h221-${index}`}>
                  <Circle
                    ...
                    fill={
                      station.intersaction3
                        ? routeTMC.highlighted || routeHovey.highlighted
                          ? '#4caf5050'
                          : '#4caf50'
              ...
              return (
                <G key={`tmc-${index}`}>
                  <Circle
                    ...
                    fill={
                      station.intersaction3
                        ? routeH221.highlighted || routeHovey.highlighted
                          ? '#1976d250'
                          : '#1976d2'
                        : 'white'
                    }
            ...
            {routeTMC.stops.map((station, index) => {
              ...
              if (station.intersaction3 && (routeH221.highlighted || routeHovey.highlighted))
                return null;
              return (
                <SvgText
                  key={`tmc-text-${index}`}
              ...
              return (
                <G key={`bus-${index}`} x={item[1][0] - 10} y={item[1][1] - 10} scale={20 / 576}>
                  <Path
                    ...
                    fill={
                      item[0] == routeTMC ? '#1976d2' : item[0] == routeH221 ? '#4caf50' : '#e53935'
                    }
                  />
                  <Path
                    d="M192 64C139 64 96 107 96 160L96 448C96 477.8 116.4 502.9 144 510L144 544C144 561.7 158.3 576 176 576L192 576C209.7 576 224 561.7 224 544L224 512L416 512L416 544C416 561.7 430.3 576 448 576L464 576C481.7 576 496 561.7 496 544L496 510C523.6 502.9 544 477.8 544 448L544 160C544 107 501 64 448 64L192 64zM160 240C160 222.3 174.3 208 192 208L296 208L296 320L192 320C174.3 320 160 305.7 160 288L160 240zM344 320L344 208L448 208C465.7 208 480 222.3 480 240L480 288C480 305.7 465.7 320 448 320L344 320zM192 384C209.7 384 224 398.3 224 416C224 433.7 209.7 448 192 448C174.3 448 160 433.7 160 416C160 398.3 174.3 384 192 384zM448 384C465.7 384 480 398.3 480 416C480 433.7 465.7 448 448 448C430.3 448 416 433.7 416 416C416 398.3 430.3 384 448 384zM248 136C248 122.7 258.7 112 272 112L368 112C381.3 112 392 122.7 392 136C392 149.3 381.3 160 368 160L272 160C258.7 160 248 149.3 248 136z"
        ...
        <Animated.View style={[styles.buttonBox, { right: 110 }]}>
          <Shadow startColor={'#00000010'} endColor={'#00000000'} distance={10}>
            <TouchableOpacity
        ...
        <Animated.View style={[styles.buttonBox, { right: 20 }]}>
          <Shadow startColor={'#00000010'} endColor={'#00000000'} distance={10}>
            <TouchableOpacity
              style={styles.buttons}
              onPress={() => {
                setVB((v) => ({ ...v, vw: VIEW_W, vh: VIEW_H, vx: -150, vy: 170 }));
              }}
            >
        ...
        <Animated.View style={[styles.buttonBox, { left: 20, width: 150 }]}>
          <Shadow startColor={'#00000010'} endColor={'#00000000'} distance={10}>
            <View
              style={[
                styles.buttonBox,
                { top: 0, width: 150, backgroundColor: '#fff', height: 70, borderRadius: 10 },
              ]}
            >
        ...
        >
          ...
          >
            <View
              style={styles.stationDetail}
              onLayout={(e) => setSheetH(e.nativeEvent.layout.height)}
            >
              <ToggleStation
                selectedStation={selectedStation}
                isHoliday={isHoliday}
                onOpenTable={(station, route) => toggleTable(station, route)}
              />
        ...
        <Animated.View
          style={{
            ...
            left: 0,
            bottom: -45,
            transform: [{ translateY: tableOffsetY }],
          }}
        >
          <Shadow
...
const styles = StyleSheet.create({
  box: { width: 10000, height: 10000, backgroundColor: '#fff' },
  //buttons
  buttonBox: {
    position: 'absolute',
    display: 'flex',
