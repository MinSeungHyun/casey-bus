...
export default function DepartingBusBlock({
  ...
  const Points =
    route == routeTMC
      ? '-120,450 0,450 0,510 -120,510 -120,450'
      : route == routeH221
        ? '-120,530 0,530 0,590 -120,590 -120,530'
        : '-120,610 0,610 0,670 -120,670 -120,610';

  const Cy = route == routeTMC ? 480 : route == routeH221 ? 560 : 640;

  const Color = route == routeTMC ? '#1976d2' : route == routeH221 ? '#4caf50' : '#e53935';
  ...
  return (
    <G id="next-station-H221">
      ...
      <SvgText x={-100} y={Cy - 20} fontSize={8} fontWeight="500">
        Next:
      </SvgText>
      <SvgText x={-100} y={Cy} fontSize={16} fontWeight="500">
        {nextBus ? nextBus[1] : 'No more today'}
      </SvgText>
      <SvgText x={-55} y={Cy - 2} fontSize={8} fontWeight="500">
      ...
      />
      <SvgText x={-100} y={Cy + 20} opacity={0.5} fontSize={8} fontWeight="500">
        Prev:
      </SvgText>
      <SvgText x={-80} y={Cy + 22} opacity={0.5} fontSize={13} fontWeight="500">
        {nextBus ? nextBus[0] : 'No more today'}
      </SvgText>

      <Rect x={-126} y={Cy - 40} width={134} height={80} fill="transparent" onPress={onPress} />
    </G>
  );
}
